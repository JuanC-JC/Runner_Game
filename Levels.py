...
class level():
    ...
    def __init__(self,matriz,width,height,Game,Background,Name,Button_Position):
    ...
    #Movimiento de todos los bloques
    def move(self):
    ...
    #Sistema de colisiones eliminar un objeto del mapa si es necesario
    def remove(self,x):
        self.totally.pop(x)

    #actualizar el estado del mapa en ejecucion
    def Update(self):
    ...
    #se carga todo el mapa inicialmente
    def __load(self):
        ...
        for layer in self.matriz:
            ...
            for tile in layer:
                ...
                if tile == "T":
                    ...
                    block = Sprite(x,y,self.width//len(layer),self.height//len(self.matriz),capa,"Grass")

                    self.List_Bloques.append(block)

                elif tile == "*":

                    block = Sprite(x,y,self.width//len(layer),self.height//len(self.matriz),capa,"Coin")
